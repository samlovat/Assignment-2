...
/* when merge, we need a temp array (named B) to temporarily store elements of
 * the original array */
int* B;
...
/* we do not really use this function anymore, but somehow we still keep it
 * here. */
...
void printA(void) {
  int i;
  printf("Array A:");
  /* FIXME: we assume the size of A is at least 100, and this will fail if it is
   * lower than 100 */
  for (i = 0; i < 100; i++) {
    printf(" %d", A[i]);
  }
...
void printB(void) {
  int i;
  printf("Array B:");
  /* FIXME: we assume the size of B is at least 100, and this will fail if it is
   * lower than 100 */
  for (i = 0; i < 100; i++) {
    printf(" %d", B[i]);
  }
...
int main(int argc, char** argv) {
  ...
  /* Now we can free the memory for the two arrays, as well as free memory for
   * arg. */
  free(arg);
  free(B);
  free(A);
